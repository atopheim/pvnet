...
def read_data(idx):
    ...
    demo_dir = os.path.join(cfg.DATA_DIR,'demo','intake3')
    source_dir = '/media/volvomlp2/03C796544677EF72/BBB/HOMEMADE/validation/renders/intake'
    #source_dir = os.path.join(demo_dir,'source')
    rgb = Image.open(os.path.join(source_dir, str(idx)+'.jpg'))
    mask = np.array(Image.open(os.path.join(source_dir, str(idx)+'_depth.png'))).astype(np.int32)
    #mask = np.array(cv2.imread(os.path.join(source_dir, str(idx)+'_depth.png'))).astype(np.int32)[..., 0]
    mask[mask != 0] = 1
    #print(mask,"-",type(mask),"-",mask.shape)
    points_3d = np.loadtxt(os.path.join(cfg.HOMEMADE, 'intake','intake_points_3d.txt')) #DEEMO DIR
    bb8_3d = np.loadtxt(os.path.join(cfg.HOMEMADE,'intake','corners.txt'))
...
def demo(idx):
    ...
    s = m.sqrt(s)
    print("--->",loss_seg.detach().cpu().numpy(),loss_vertex.detach().cpu().numpy(), precision.detach().cpu().numpy(), recall.detach().cpu().numpy(),s)
