...
train_cfg['model_name'] = '{}_{}'.format('bypass-v', train_cfg['model_name'])
...
def read_data(idx):
    ...
    source_dir = '/media/volvomlp2/03C796544677EF72/BBB/HOMEMADE/renders/bypass-v/'
    ...
    points_3d = np.loadtxt(os.path.join(cfg.HOMEMADE, 'bypass-v','bypass-v_points_3d.txt')) ##DEEMO DIR
    bb8_3d = np.loadtxt(os.path.join(cfg.HOMEMADE,'bypass-v','corners.txt'))
...
def demo(idx):
    ...
    #########NBNBNB#############
    model_dir = os.path.join(cfg.MODEL_DIR, "bypass-v_demo") #<-##############################################################3
...
if __name__ == "__main__":
    for idx in range(0,10):#(3482,3500):
        demo(idx)