...
from lib.utils.data_utils_homemade import read_pickle, HomemadeImageDB, read_rgb_np, read_mask_np, OcclusionHomemadeImageDB, \
    HomemadeModelDB
import os
import torch
...
class VotingType:
    ...
    def get_pts_3d(vote_type,class_type):
        homemade_db=HomemadeModelDB()
        if vote_type==VotingType.BB8C:
            points_3d = homemade_db.get_corners_3d(class_type)
            points_3d = np.concatenate([points_3d,homemade_db.get_centers_3d(class_type)[None,:]],0)
        elif vote_type==VotingType.BB8S:
            points_3d = homemade_db.get_small_bbox(class_type)
            points_3d = np.concatenate([points_3d,homemade_db.get_centers_3d(class_type)[None,:]],0)
        elif vote_type==VotingType.Farthest:
            points_3d = homemade_db.get_farthest_3d(class_type)
            points_3d = np.concatenate([points_3d, homemade_db.get_centers_3d(class_type)[None, :]], 0)
        elif vote_type==VotingType.Farthest4:
            points_3d = homemade_db.get_farthest_3d(class_type,4)
            points_3d = np.concatenate([points_3d, homemade_db.get_centers_3d(class_type)[None, :]], 0)
        elif vote_type==VotingType.Farthest12:
            points_3d = homemade_db.get_farthest_3d(class_type,12)
            points_3d = np.concatenate([points_3d, homemade_db.get_centers_3d(class_type)[None, :]], 0)
        elif vote_type==VotingType.Farthest16:
            points_3d = homemade_db.get_farthest_3d(class_type,16)
            points_3d = np.concatenate([points_3d, homemade_db.get_centers_3d(class_type)[None, :]], 0)
        elif vote_type==VotingType.Farthest20:
            points_3d = homemade_db.get_farthest_3d(class_type,20)
            points_3d = np.concatenate([points_3d, homemade_db.get_centers_3d(class_type)[None, :]], 0)
        else: # BB8
            points_3d = homemade_db.get_corners_3d(class_type)
...
>>>>>>> 2a470f5ac1250bcc075a41035b5cfe8fa5b48685
        ...
        if self.imagedb[index]['rnd_typ']=='fuse':
            mask=np.asarray(mask==(cfg.homemade_cls_names.index(self.imagedb[index]['cls_typ'])+1),np.int32)
...
class ImageSizeBatchSampler(Sampler):
    def __init__(self, sampler, batch_size, drop_last, cfg=default_homemade_cfg):
